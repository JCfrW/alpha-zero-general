...
Well here it is more Othello and tested on pytorch !

Some improvment in order to be more alphazero, the nn is always the latest and add some randomization
at the root nodes in order to have different games

I added also endgame.c from Zebra which came from Warren Smith's code which where coming itself from
some code from mine.

I added also pit2.py in order to test against EDAX and othello/book/XOT from xot.xmav.eu in order
to test with differents opening moves !

See othello/pretrained_models/othello/pytorch/restart.pth.tar for an old attempt by me
and temp/restart.pth.tar for an attempt from this version of the source

Have fun!
[jcw]

# Back from the README from the Alpha Zero General
