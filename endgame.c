...
* Introduced the symbolic constant INFINI which holds a value outside
   ...
   int color, int empties, int discdiff, int prevmove )
{
   ...
   for(old_em = &EmHead, em=old_em->succ; em!=NULL;
    ...
    if(score == -INFINI){  /* No legal move found */
       if(prevmove == 0){ /* game over: */
#if WINNER_GETS_EMPTIES
          if(discdiff>0) return discdiff+empties;
          if(discdiff<0) return discdiff-empties;
...
static PyObject *py_endsolve(PyObject * self, PyObject *args)
{
	char * bds;
	PyArg_ParseTuple(args, "s", &bds);
	int wc, bc, emp, x, y, k;
	int j;
	...
	PrepareToSolve( board );
	int val = EndSolve(board, -1, 1, WHITE, emp, wc-bc, 1);
...
static PyMethodDef py_methods[] = {
		{"solve", py_endsolve, METH_VARARGS}, {NULL, NULL}};

void initendgame()
{
	(void) Py_InitModule("endgame", py_methods);
};
